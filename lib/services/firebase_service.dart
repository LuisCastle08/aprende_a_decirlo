...
Future<Map<String, dynamic>> getUsuario(String userId) async {
  ...
  QuerySnapshot queryUsuario = await db.collection('usuario').where(FieldPath.documentId, isEqualTo: userId).get();
...
Future<void> addComentario(String comentario, String name, int tipoComentario) async {
...
Future<void> updateComentario(String uid, String newName) async {
  await db.collection('usuario').doc(uid).set({"nombre": newName, "comentario" : 'HOLA SOY LUIS'});
